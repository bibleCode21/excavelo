...
| 2026-07-28 | A branch a landing names (by merge-parse or path 1) is reported by name whatever the window does: narrowing `since:`/`until:` never drops it, only whether its commits render alongside it — an out-of-window or cap-evicted named landing still surfaces a dated, header-only `--- confirmed landed on <base> branch: <name> (landed <date>)` section, capped and noticed like every other section kind. Qualifies the 07-27 row | Measured on the 07-27 unit's own merge: a selected branch confirmed by name still reported nowhere when the window did not cover its landing — `branches:<glob>` with no window (the 7-day default reaching named landings too), an explicit window on a branch paths 2/3 both fail, and a branch whose only pasted spelling was its ref form while the landing was merge-parsed under its display form. The first shape alone silenced every branch older than a week under a glob's own default. Deferred out of 07-27 rather than folded in — the fix pulls in a window-invariance rule, a new cap, and a return-shape change, and that unit's round cap was already spent. Contract: docs/specs/git-log-named-window-invariance.md |
| 2026-07-29 | A name that is the default branch's own is not a name: a merge subject parsing to the base (`Merge pull request #N from someuser/main`, ordinary whenever a contributor's fork also defaults to `main`) leaves its landing nameless, rendered as `--- landed <date> merge: <subject>`. Nameless in full — such a landing takes the nameless window and cap priority, so under a pasted selection with no explicit window it is now bounded by the 7-day default rather than reported unbounded under a false name. Qualifies the 07-28 row | The base was being reported as a branch that landed on itself, which the 07-27 row's predicate forbids by every other path. The rule had been written on the *readers* of a landing's name and was missing from two of them — a full panel caught one, and this unit found the other still reporting it with no selection in play at all; it now lives at the two write sites instead, where the count is fixed. Accepted cost, stated rather than discovered: a base-naming merge older than the nameless default disappears from an unpulled clone's pasted selection, where it previously rendered mislabelled, and appears under a narrow selection that never matched the base, where it was previously withheld. Contract: docs/specs/git-log-base-named-merge.md |
| 2026-08-01 | "Nothing that exists matched this glob" and "what matched carried a name that was not usable" are different facts, and only the first is an error: a `branches:<glob>` matched by a base-naming merge's parsed subject renders that repository as a selection instead of raising `git.no-branches`. The widened question is asked at both of the gate's sites — the throw and the selection-mode decision beside it — and is scoped to glob mode, so pasted-candidate mode is byte-identical. Two sentences of the 07-29 row's contract are corrected here rather than in place, since editing a confirmed contract would void its confirmation: the path-1 write guard rejects a real input (`git-log-base-named-merge.md` §Spec says it "rejects nothing"), and `names`' base exclusion is not load-bearing (its §Non-goals says it is). Qualifies the 07-29 row | Measured both sides of that unit's merge: at `5f82ec9` a callout whose glob was spelled in the base's *ref* form rendered `--- landed 2024-03-01 branch: origin/main` and `--- landed 2024-01-01 direct`; at `66a667d` it rendered nothing at all, and because `loadGitLog` throws for the whole spec list rather than for one spec, every other repository in the same callout lost its output too. Dropping the false header was the fix working; losing the base's own legitimate landing, and the rest of the callout with it, was not. The two corrections travel with the fix because all three are consequences of the same move and each is measured rather than argued: replacing `landingName(base, displayOf(name))` with `displayOf(name)` renders `--- landed <date> branch: main` while all 175 existing checks stay green — a live guard with no net, now held by F1 — and deleting `names`' base exclusion leaves all 175 green as well, so it stays as what it is, an input narrowing, not the guard its comment claimed. Contract: docs/specs/git-log-base-guard-pinning.md |
| 2026-08-02 | Two more sentences of `git-log-base-guard-pinning.md`'s (`ddd71b41`) own text are corrected here rather than in place. §Spec's gate section attributes the pasted-mode fall-through pin to E6 ("`git-log-base-named-merge` §Spec's disappearance direction (its E6) pins that"), but §Preservation contract states the opposite two paragraphs earlier in the same document: E6's fixture leaves the base as `{display:"main", ref:"origin/main"}`, so `selectBranches` selects it and the gate is never reached — E6 pins the *selected* case, not the fall-through; the actual pin is **F5**. Separately, §Spec's F1 bullet claims its mutation ("replacing `landingName(base, displayOf(name))` with `displayOf(name)`") "turns F1 red, and turns no other check red" — no longer true once §Invariants' own requirement, stated two paragraphs later, folds F1's and F2's outputs into the `M1` sweep: the same mutation now reddens M1 alongside F1. Qualifies the 08-01 row | Measured against the merged code, not argued from the prose: the E6 fixture leaves the base selected (gate unreached) while F5's fixture takes the plain path (gate reached), and only F5's mutation — widening the question without the `spec.branches` scope — reddens; the F1 mutation reddens F1 and M1 together, checked directly, where the contract's text says F1 alone. Contract: docs/specs/git-log-base-guard-pinning.md |
| 2026-08-02 | Two sentences of `release-metadata-invariants.md`'s (`0860dac`) R3 are corrected here rather than in place. §Spec says counting the heading itself in R3's body scan "would... make the body check unfailable" — the opposite is measured: counting the heading makes the terminator search match at index 0, so `body = []` and the check fails on every input tried, not none. §Spec's mutation-table note also states a duplicated `## [<version>]` heading always extracts "0 bytes" via `release.yml`'s awk — also not general: the awk's first rule (`$0 ~ "^## \[" tag "\]" {p=1; next}`) swallows a same-version duplicate with its own `next`, so extraction stays non-empty in that shape; the actual condition is whether a non-blank line sits between the first tag heading and the next *different* release heading. Five prose attempts inside the original unit to state that condition were each measured false in turn; the unit closed by deleting the causal sentence and keeping only the assertion, and the same treatment is recommended here rather than a sixth attempt | Measured directly against `release.yml`'s awk: counting-the-heading failed on every input tried (not "unfailable"), and duplicate-heading extraction is non-empty whenever the awk's own `next` absorbs the duplicate before the exit rule can see it. `release.yml`'s empty-note guard and the probe's "exactly one heading" check are what actually close this gap; the contract's causal story about *why* was wrong on both edges. Contract: docs/specs/release-metadata-invariants.md |
| 2026-08-02 | One sentence of `git-log-base-named-merge.md`'s §Why ("Refactor rationale") is corrected here rather than in place: "TypeScript cannot restrict the field to one writer without a class or module boundary, which a 1200-line file does not warrant" is false on both halves. A branded/nominal type on `Landing.branch` can make the compiler enforce single-writer discipline without a class or module boundary, so the impossibility claim doesn't hold; and line count is not what would make a boundary unwarranted — write-site count is (this file has two, both already known and named), where a much shorter file with many writers would want the boundary more, not less. The decision itself — a stated invariant plus a helper, not a class — stands and is not revisited; only its stated reason is wrong. Qualifies the 07-29 row | The decision was cost/benefit for two known writers ("we chose not to"), not a compiler limitation, and citing an impossibility that isn't one invites a future reader to accept the same reasoning where it is actually false. Contract: docs/specs/git-log-base-named-merge.md |
| 2026-08-18 | One clause of `git-log-base-guard-pinning.md`'s (`ddd71b41`) `allowed-surface` parenthetical is corrected here rather than in place: "Deferred item 16 records why this qualifier is written out — the deciding unit is still open" no longer holds, since that unit ran on 2026-08-18 and fixed the binding unit at the path. The rest of the parenthetical is **ratified rather than overturned** — "the binding unit of this list is the path… the prose after each dash describes intent and is not a closed enumeration" is now normative for every contract in every profile, written into the global `work-contract-format.md`'s `allowed-surface:` definition and into `governance-review.md`'s Surface check, so the qualifier no longer has to be written out per contract; where it already is, it stays useful prose that simply does not narrow the gate. Qualifies the 08-02 row | The qualifier was a per-contract workaround for an under-specified gate: `work-contract-format.md` defined the field as "paths/files this work may edit" while governance read each entry's em-dash prose as a closed set. Measured cost: a BLOCK→amend→spec-review→re-approval→re-pin cycle every time a contract added a check its own §Invariants implied — twice inside `git-log-base-named-merge` alone, with a third case (`767e8da`'s two fixtures) flagged by governance. ADR-0006 never fixed a binding unit, so stating it closes an under-specified point rather than reversing a decision, and no new ADR was written; both sites were amended together because amending the format doc alone would leave the gate written-only, ADR-0006's own three-site precedent. Harness commit `305a7aa`. Contract: docs/specs/git-log-base-guard-pinning.md |
| 2026-08-18 | Two commit messages on `feature/probe-git-log-split` are corrected here rather than by rewriting history. `029a27a` says its `approved-commit` was "matching the profile approval ledger line recorded in the same step", and `5ea6f97` says "the profile approval ledger line was rewritten in the same step, not appended". Both describe the ledger *file* correctly and the ledger *commit* falsely: when each was written the line existed only as an uncommitted working-tree edit in the `claude-profiles` repo, and it first reached a commit 28 minutes later, in `adf3f92`, prompted by the governance leg refusing to gate against a pin it could not verify. The pin itself is sound and unchanged (`probe-git-log-split@bb135d0`, one line for one id, matching the frontmatter). What was wrong is the tense: a cross-repo write is not done until its own commit exists, and a message must not assert one that has not been made | The gate behaved correctly and that is worth recording as such: `PROD_RE` is `^src/\|^(manifest\|versions)\.json$`, so the two commits that landed before `adf3f92` (`ff9c8d8`, `5ce9d19`) touched `scripts/` only and took the non-prod fast path by design, while the unit's one prod commit (`f1085e3`, a comment in `src/core/git-log.ts`) was BLOCKed by governance for exactly this missing ledger line and landed only after `adf3f92` supplied it — the fail-closed pin check firing where ADR-0016 intends. The residual defect is a message asserting a state it had not yet produced: cheap to avoid (make the ledger commit first, then quote its hash) and invisible to every automated check, since no hook reads commit prose. Contract: docs/specs/probe-git-log-split.md |
| 2026-08-18 | One cell of `probe-git-log-split.md`'s (`bb135d0`) §Spec module table is corrected here rather than in place: the `selection-and-traversal.mjs` row says `tryLoad` "stays here beside the `matches` helper its comment names", and the delivered tree has it in `harness.mjs`. The full-panel design lane found the placement wrong — `tryLoad` depends on nothing but `loadGitLog`, which the harness owns, and three of the five check modules were importing a generic error-swallowing wrapper *from a check module* to get it. The comment that justified the original placement no longer had to point sideways once it moved, and was rewritten to say what actually distinguishes `tryLoad` from `matches` (a captured throw to assert against, versus a boolean with a rethrow). The contract's `allowed-surface` is unaffected: its binding unit is the path and `scripts/probe-git-log/` is listed whole — the table cell is intent prose, which the 08-18 row above made normative as non-binding | The table was written before the modules existed, so the placement claim was a plan, not a measurement; the panel measured the coupling it produced and it was the one import in the delivered tree that revealed a boundary in the wrong place. Correcting the row in place would void the contract's confirmation and force a re-review and re-pin for a cell that was never a gate input — the same trade the 08-01 and 08-02 rows record. Contract: docs/specs/probe-git-log-split.md |
