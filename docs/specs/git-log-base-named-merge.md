...
- allowed-surface:
  ...
  - `scripts/probe-git-log.mjs` — E1–E7; `buildBaseNamedMergeFixture`'s second base-naming merge, its `feature/x` ref and its remote-base arm; and the stale comment above the existing B12 fixture call that records the wide-window case as deferred.
