...
status: confirmed
ceremony: standard
approved-commit: e8db4103ef4e45061e1e7b8ac9da558f15488a40
---
...
2. `eslint.config.mjs`: 전면 재작성.
   ...
   - overlay 1 — `files: ["src/**/*.ts"]`, `languageOptions.parserOptions: { project: "./tsconfig.json", tsconfigRootDir: import.meta.dirname }`, `rules`에 3개 오버라이드(`@typescript-eslint/no-unused-vars: [error, {args: none}]`, `no-prototype-builtins: off`, `@typescript-eslint/no-empty-function: off`) 보존. `obsidianmd/no-unsupported-api` 명시적 오버라이드는 포함하지 않는다(recommended 기본값 신뢰). — **amendment(feature-complete 패널 이후)**: 원래 기존 5개 오버라이드 전부 보존 계획이었으나, feature-complete 패널(design-review)이 `no-unused-vars: off`/`@typescript-eslint/ban-ts-comment: off` 2개는 `obsidianmd.configs.recommended`(`flatRecommendedGeneralRules`)가 이미 동일 값으로 설정하고 있어 순수 중복임을 실측 확인(설치된 패키지 소스 직접 확인) — 삭제해도 `pnpm lint` 결과(0 error / 4 warning) 불변임을 재검증 후 제거. 남은 3개는 각각 실질적 의미가 있음: `no-unused-vars`는 심각도를 error로 올림, `no-prototype-builtins`는 플러그인 자체의 오타(`no-prototype-bultins`)를 보정, `no-empty-function`은 recommended에 아예 없는 항목.
