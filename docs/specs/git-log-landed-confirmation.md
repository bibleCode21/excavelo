...
- **Preservation contract** — `git-log-master-source`'s **A1, A3, A4, A5, A6, A7, A8, A10, A11, A12, A13, A14, A16 and I1, I3, I4 survive**, three of them with their scope made explicit below rather than changed. The following criteria **do not survive verbatim** — this is the complete enumeration, and any criterion not listed here is preserved as written:
  ...
  - **A6 / A13** are not in tension and neither is narrowed: a **spec-supplied** `since:`/`until:` bounds every landing kind (A6), and the **default** window is what named landings in *pasted* mode never receive (A13). The new bound adds `DEFAULT_SINCE` for nameless landings only, and only when the spec supplied neither token.
  - **The predecessor's glob-mode default window is preserved**, explicitly: its §Why kept the 7-day default for `branches:<glob>` as "today's behavior, preserved as-is", and this contract does not re-open it. An earlier draft of §Windowing generalised "named landings take no default" across both modes and would have removed it without listing it here — that is exactly the silent narrowing this table exists to prevent, and it is called out because the table claims to be complete.
...
**Path 1 — the name appears in a landing's message.** For each candidate name, search the landing messages (subject + body) read **without a window**, matching the name as a **whole token**: an occurrence counts only when neither the character before it nor the character after it is one git allows in a ref name (`[A-Za-z0-9._/-]`). The branch is confirmed when the matching set, after the exclusions below, contains **exactly one** landing. Requires no branch ref: the name is a string, so a branch deleted after its squash landing is still confirmable. The matched landing is then labelled with that name and rendered by the existing `loadLandingSections` path — `--- landed <date> branch: <name>`, commits included.

> Rationale for the exactly-one bound: `branchCandidates` accepts any slash-bearing token, so a memo mentioning `src/core/git-log.ts` puts a *file path* through this search, and file paths recur across many commit messages. Requiring uniqueness rejects them. This also preserves A12.
>
> Rationale for the token bound: a bare substring cannot tell `feature/n1` from a message naming `feature/n10`, and ticket-prefixed conventions — the measured repository's `0713-sr2607-00334-kjh0407` shape — make sibling names routine, so without it a whole family of real branches becomes unconfirmable. It also narrows the threat below.

> **Accepted threat: a landing message is third-party text, and paths 1 and 2 both let it decide.** Anyone able to land one commit on the base — a colleague, or an upstream the user fetches — can write another person's branch name into that commit's message, and path 1 will then report that person's *unlanded* branch as shipped, carrying the planted commit's body. The token bound removes the accidental case (a name appearing inside a longer path, URL or identifier) but **not** the deliberate one, and it cannot: the whole point of path 1 is that a message naming a branch is evidence. Non-adversarial reachability is the same mechanism — `undo the feature/x change` or a squash body that swept up `Merge branch 'main' into feature/x` both confirm.
>
> **Path 2 is reachable the same way and is not protected by its stricter bound.** It matches commit subjects as substrings of the same third-party messages; "every subject must resolve" raises the cost in proportion to how many commits the branch carries, but a one-commit branch needs exactly one planted mention, and its subject is public to anyone who can see the branch. No token bound applies there, because a subject is a sentence rather than an identifier and has no delimiter to key on.
>
> Both are accepted rather than closed because the alternative is discarding the only signals that see through a squash — stated here so they are known residuals and not unexamined ones. The exclusions below bound the commonest false positive (a revert quoting its original).

**One landing carries at most one name.** A landing that already has a name (parsed from its merge subject, or assigned by an earlier candidate) keeps it; a later candidate matching the same landing confirms nothing and emits nothing. This makes the rendered name **deterministic when two distinct candidates match one landing** — first-in-memo wins rather than last. (It is *not* what keeps two spellings of the same branch from double-labelling: "Which name is rendered" below folds `feature/x` and `origin/feature/x` to one name before this rule is reached, and a landing is one object rendered once either way, so I3 is not at stake here.)
...
2. **Full resolution.** `git log <base>...<branch> --right-only --cherry-pick --pretty=%s`, windowless, yields the unresolved subjects. Each resolves when it occurs as a substring of **exactly one** landing's message, after the exclusions below. The branch is confirmed when the unresolved count reaches zero. **Empty subjects count**: a commit carrying no subject can never resolve, and discarding it would let the count reach zero by dropping the one commit that could not be accounted for — a branch with unlanded work reported as shipped.
...
> **Exit codes are an answer here, not a failure**, and this module has no precedent for that — every other git call treats non-zero as fatal (`git-log.ts:298, 416, 495-498` → `git.failed`), and `git-log.ts` contains no `merge-base` today. Exit **0** = confirmed, exit **1** = not an ancestor (a normal negative, never surfaced as an error), any other exit (128 = bad ref, or null when the child dies on a signal) keeps the existing `git.failed` contract, which §Non-goals preserves — as does a *rejection* from the spawn layer (timeout, ENOENT), which both new query helpers must map onto `git.failed` the way every other call site does.

> **The two revisions are passed after `--`.** A ref may legally be named `-evil/x`; without the separator git reads it as an unknown switch, exits 129, and the whole transform fails for every repository in the callout.
...
**The base ref is excluded from the predicate entirely** — not just from path 2. `merge-base --is-ancestor <base> <base>` exits 0, so without the exclusion the base confirms itself through path 3; `branches:*` selects it and `origin/master` is a valid pasted candidate. This also keeps it out of every cap slot, as today.

**Excluding it by raw ref string is not enough**, and this is the one place the base's *identity* has to be reasoned about rather than compared. `enumerateBranches` deduplicates by `display` and, on a committer-date tie, keeps the ref it enumerated first — `refs/heads` precedes `refs/remotes` — so an ordinary clone whose local `main` tracks `origin/main` lists `{display:"main", ref:"main"}` while `resolveBaseRef` returns `origin/main`. `ref !== base` then passes and the base confirms itself. The base must therefore carry its **display form** too (the same remote-prefix rule `enumerateBranches` applies), and a branch is excluded when either form matches. Excluding by *commit* instead is wrong here: a fast-forwarded branch sits on the base's own commit, which is exactly the shape path 3 exists for.
...
| landing kind | spec supplied `since:` and/or `until:` | spec supplied neither |
|---|---|---|
| named (merge-parsed, or named by path 1), **pasted mode** | bounded by it (A6) | unbounded — no default (A13) |
| nameless (`direct`), **pasted mode** | bounded by it (A6) | bounded by `DEFAULT_SINCE` — **the new bound** |
| either kind, **`branches:<glob>` mode** | bounded by it (A6) | bounded by `DEFAULT_SINCE` — **unchanged from today** |

**"Supplied a window" means either token, not `since:` alone — in both modes.** A spec carrying only `until:` has supplied one, so no default is added on top: pairing `DEFAULT_SINCE` with a past `until:` yields `7 days ago .. <that date>`, an empty range that deletes the landings it was meant to bound. Which landings it deletes differs only by which side the default was being applied to at the time: in pasted mode the default reaches the nameless side, so that side empties — in the repository shape this contract exists for, where nameless landings *are* the work. In glob mode the default reaches the named side too, and the named side is what empties. Both are the same defect and neither is acceptable; the rule is one rule, and B17 pins it in both modes because covering one is how the first version of it survived a full panel.

**Glob mode keeps its 7-day default.** The predecessor contract's §Why held this explicitly ("that asymmetry is today's behavior, preserved as-is, and re-opening it is not this work's business"), and re-opening it is still not this work's business: without it a glob with no window renders up to `MAX_BRANCHES` named landings × up to `MAX_COMMITS` commits with diffstats from all of history — the same prompt bloat this contract closes for nameless landings, in the one mode where the user supplied no bound at all. Only pasted mode has the "an explicit paste must not be silently bounded to a week" argument behind it, because only there does the user name the specific branches.
...
The repository header keeps reporting the window the *spec* set, so A13's `(all history` assertion survives; the nameless default is named as a second clause only when it is actually in force (pasted mode, no spec window).
...
`MAX_BRANCHES` (50) caps each section kind independently, as today.

**The cap bounds the scan, not the confirmations.** At most `MAX_BRANCHES` selected branches are *examined*; the rest are not queried at all. Gating on the number confirmed instead would leave a selection where nothing confirms iterating every selected branch and spawning one or two sequential git processes for each — and a shared remote's branch count is not this file's to trust, since anyone can push to it. This is the predecessor's `considered.slice(0, MAX_BRANCHES)` kept in place, not a new bound.

Path 1 costs nothing per candidate: the landing messages are already in memory from the predicate's walk, so matching every name in one pass over them replaces one `git log --grep` per candidate and its spawn. That is the cost claim only — the *verdict* is deliberately not git's: `-F -i --grep` matches a bare substring, and the token bound above narrows that on purpose. B13 pins the difference, so this is not a scan that may be "simplified" back to a substring match. Path 2 costs up to two `git log`s per examined branch (the floor query and the cherry-pick query); path 3 costs one `merge-base` per branch neither confirmed. Landings are read **at most twice** per repository. Once unwindowed, for the predicate. Then at most one windowed read for rendering: when the spec supplied a window, the named and nameless sides share it and one read serves both; when it supplied none, the named side takes no windowed read at all (it is unbounded in pasted mode, and in glob mode its bound is the same default the nameless side takes), so again one read. No arrangement reaches three. `MAX_COMMITS` and `GIT_TIMEOUT_MS` keep their meaning per invocation.

Two over-cap notices survive, in the existing shape: the landing-side notice unchanged, and the branch-side notice repurposed to report how many selected branches were **examined** out of how many were eligible. It fires when the scan was truncated, never merely because the cap number was reached — a run that examined every eligible branch reports nothing, whatever it confirmed.

### Acceptance criteria

Extending `scripts/probe-git-log.mjs` and its fixture (new shapes: a squash landing whose message body carries the source branch name; a squash landing whose subject was rewritten; a branch deleted after landing; a landing subject that recurs; a prefix/longer branch-name pair; and a second repository shaped like an ordinary clone, with a remote base and a local ref of the same display name).
...
- **B11.** Pasting both `feature/x` and `origin/feature/x` for one landing yields exactly one section for it, headed with the ref's `display` form — the rendered-name rule pinned. (Not an I3 criterion: the two spellings fold to one name before any rule about duplicate labelling is reached.)
- **B12.** A candidate naming the base ref itself is not confirmed and emits no section, by any path.
- **B13.** The token bound holds in both directions: a branch whose name is only a **prefix** of the name a landing mentions is not confirmed by it, while the longer name it is a prefix of still is. Pinned through `loadGitLog` against a `feature/tok` / `feature/token-longer` pair — **not** by exporting the matcher, on A14's precedent (widening the module's public surface to make something testable is not licensed).
...
- **B16.** `merge-base --is-ancestor` exiting 1 produces no section and no error; a bad ref (exit 128) still raises `git.failed`, and it is the predicate that raises it, not the landing walk.
- **B17.** An `until:`-only window adds no default, **in both selection modes**, and still bounds what it covers: a landing dated inside it appears, one dated after it does not, and the header announces no default. In pasted mode the assertion is on nameless landings; in glob mode it is on named ones, since that is the side a glob's default bounds. With no window at all the same landings fall outside `DEFAULT_SINCE` instead — the two bounds are independent. Written for one mode and exercised on one mode is how the first version of this rule survived a full panel; both modes are required here.
- **B18.** In a repository whose base resolves to a remote ref while a local ref of the same display name exists — an ordinary clone — the base branch confirms nothing and is named nowhere. The fixture must plant `refs/remotes/origin/main` plus `refs/remotes/origin/HEAD` alongside a local `main`; every other fixture uses a local base, which is why this shape had no coverage.
- **B19.** `branches:<glob>` with no `since:` reports the 7-day default window, not all history.
- **B20.** A `--- landed` line planted after a VT, FF or NEL byte in a commit message is escaped, like one planted after a newline. `escapeMarkerLines` treats those three as line starts because git keeps them verbatim and a renderer downstream may break on them; with not-yet-landed sections gone, a forged section is believed outright rather than merely competing with a real one.
...
- allowed-surface:
  - `src/core/git-log.ts` — the predicate, its three paths, the new label, the window split between named and nameless landings, `loadNotLandedSections` inverted. Also `escapeMarkerLines`: its docstring enumerates the reserved header syntax and must name the third label, and **the regex itself widens** to treat VT, FF and NEL as line starts alongside the terminators `^` already recognises under `/m`. That is a security control being widened, not a cosmetic edit — git keeps those bytes verbatim in a commit message, and this contract removes the landed-wins rule that used to keep a forged section merely competing rather than believed. B20 pins it.
  ...
  - `scripts/probe-git-log.mjs` — fixture shapes, B1–B20, and the two rebuilt/re-anchored unnumbered nets.
