...
**조치 (WU-0)**: `obsidianmd/obsidian-releases`에 케이싱 수정 PR
(`biblecode21/excavelo` → `bibleCode21/excavelo`). PR 후에도 실패가 재현되면
Obsidian 지원 채널로 검증 로그 요청. (대안: GitHub 계정명을 소문자로 변경 —
파급이 커서 비권장.)
...
1. WU-0 머지 후 심사 대시보드에서 attestation Error 소멸.
...
### WU-0 — 등록부 케이싱 수정 (P0, 코드 외, 즉시)
`obsidianmd/obsidian-releases`에 단일 라인 PR. 근거·재검증 절차는 §1.1.
검증: PR 머지 후 심사 재실행에서 attestation Error 소멸.
...
- WU-0 가설이 틀렸을 경우(봇이 다른 이유로 실패): PR 비용은 1라인이며 어차피
  등록부 오기는 사실이므로 손실 없음. 실패 재현 시 Obsidian 지원 문의로 전환.
