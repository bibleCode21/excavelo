...
status: confirmed
ceremony: standard
approved-commit: 8755f2a47f945f5da63bd2fe487971a2d1d33a58
---
...
- A14. **Star runs collapse.** A `branches:` glob with a star run, against a fixture whose merge subject carries a ~100k-character branch name, returns in well under a second. Reached through `loadGitLog` exactly as A10's globs are: `globToRegExp` is not exported, and **must not be exported to make this testable** — §Why preserves its semantics, and widening the module's public surface for a test is not licensed. This exercises the path the security finding named — a third party's commit subject reaching the user's glob — rather than the function in isolation.

  The concrete glob and name are the probe's to choose, but **the choice must have all four properties below, or the criterion passes against the very code it exists to catch.** Stated as properties rather than one fixture because more than one shape satisfies them (measured, uncollapsed vs collapsed: `**/hotfix` vs 100k×`a` → 11529ms / 0.7ms; `feature/**/fix` vs `feature/`+50k×`a` → 3141ms / 0.4ms):
  - **A literal after the star run.** `**` alone compiles to `^.*.*$`, which has nothing to fail against: the greedy first pass succeeds and never backtracks — 0.2ms either way, *vacuous*. Backtracking needs a literal the input fails, forcing every split of the input between the two `.*` to be tried.
  - **The name must fail that literal.** A name that matches would return on the first pass.
  - **It must fail late, not at a prefix.** A name failing at a literal *prefix* is 0.0ms uncollapsed — the regex never engages. "A non-matching name" is not enough, and a criterion saying only that measures nothing. (This is the trap that caught both the author and the reviewer of this contract; it is written down because measuring it wrong looks exactly like measuring it right.)
  - **The long text must reach the glob.** It only does through a subject `parseMergeBranchName` can parse — a merely-long subject returns `null`, `match(null)` short-circuits, and the regex never runs. A branch ref matching the glob must also exist, or `git.no-branches` throws before the name-filtering step.
  - **The margin must survive slow hardware.** The threshold is the probe's to pick, not this contract's — but keep the fixture's non-regex overhead (git spawns, fixture build) at least an order of magnitude below it. The discriminating gap belongs to the regex, not the harness: the signal is four orders (0.1–0.7ms collapsed vs 7.6–11.5s uncollapsed, on two machines that differ ~1.6× from each other), while a passing run is almost entirely spawn overhead. Get that wrong and the criterion dies by flaking into deletion on a slower machine rather than by passing vacuously — a different death, the same loss. When the margin is thin, cut the overhead; do not raise the threshold, which attacks the signal instead of the noise.

  (A10's globs carry a single star each, so without A14 a revert of the collapse passes every other criterion.)
