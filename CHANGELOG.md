...
- **A `[!git]` path starting with `~<name>` is left alone instead of being mangled.** Only `~` and `~/…` resolve to your home directory; `~someone` was previously glued onto the home path without a separator, producing a nonexistent directory rather than leaving the unsupported syntax untouched.
