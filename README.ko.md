...
**작업은 기본 브랜치에 반영(landing)될 때 완료로 칩니다.** 브랜치에 커밋만 되어 있으면 완료가 아닙니다. ExcaVelo 는 기본 브랜치를 훑어 거기 도착한 것을 반영 건별로 한 섹션씩 보고하며, 날짜는 작업한 날이 아니라 **반영된 날**입니다. 아직 반영되지 않은 브랜치의 작업은 아예 보고되지 않습니다. merge / squash / rebase 어느 방식이든 동작하며 설정할 것은 없습니다.

git 출력에서 복사한 브랜치 줄(`브랜치명  커밋제목`)을 메모에 붙여넣으면, ExcaVelo 가 그 브랜치명을 달고 있는 반영 건만 정확히 보고합니다 — 이슈당 브랜치 하나로 일하는 팀에 맞는 방식입니다. `work-report` 와 `work-log` 템플릿이 이 입력용으로 만들어졌습니다.

브랜치명은 팀이 merge 커밋으로 병합할 때만 히스토리에 남습니다. squash / rebase 방식이라 이름이 남지 않으면, ExcaVelo 는 붙여넣은 브랜치가 기본 브랜치에 실제로 반영됐는지를 세 가지로 순서대로 확인합니다: 반영 건의 메시지가 그 브랜치명을 담고 있는지(브랜치를 병합 후 삭제했어도 잡힙니다), 브랜치의 각 커밋 제목이 정확히 하나의 반영 건에 대응하는지, 아니면 브랜치가 이미 기본 브랜치 이력의 일부인지.

셋 중 무엇으로도 확인되지 않은 브랜치는 **어디에도 보고되지 않습니다.** 의도된 동작입니다 — squash 가 흡수하면서 커밋 제목을 새로 쓴 경우 따라갈 근거가 남지 않는데, 추측으로 "완료"라고 적는 것은 빠뜨리는 것보다 나쁩니다. 이름 없는 반영 건들은 창(직접 지정하지 않으면 최근 7일) 안에서 그대로 함께 보고되므로, 작업 자체가 사라지는 것이 아니라 브랜치와의 연결만 빠집니다. 붙여넣은 브랜치는 반영 시점이 아무리 오래됐어도 찾아냅니다.
