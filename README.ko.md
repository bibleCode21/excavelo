...
브랜치명은 팀이 merge 커밋으로 병합할 때만 히스토리에 남습니다. squash / rebase 방식이라 이름이 남지 않으면 반영 건 자체는 그대로 보고되지만 두 가지가 나빠집니다: 붙여넣은 이름으로 로그를 좁히지 못하고, 한 창에 반영 건이 50개를 넘으면 **무엇을 선택했는지와 무관하게** 오래된 것부터 잘려나가므로 바쁜 저장소에서는 선택한 작업이 누락될 수 있습니다. 그런 환경이면 `since:` 로 창을 좁혀 쓰세요.
