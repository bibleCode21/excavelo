...
한 줄에 저장소 하나씩, `since:` / `until:` 은 선택 (ISO 날짜, `today`, `7d`). git 출력에서 복사한 브랜치 줄(`브랜치명  커밋제목`)을 메모에 붙여넣으면, ExcaVelo 가 그 브랜치들을 지정된 저장소에서 찾아 정확히 그 브랜치들의 커밋만 공급합니다 — 이슈당 브랜치 하나로 일하는 팀에 맞는 방식입니다. `work-report` 와 `work-log` 템플릿이 이 입력용으로 만들어졌습니다.
